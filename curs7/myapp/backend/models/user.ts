...
export const create = (user: User, callback: Function) => {
  const queryString =
    "INSERT INTO jsusers (nume, prenume, email, datanastere, telefon, inaltime) VALUES (?, ?, ?, ?, ?, ?)";
    console.log(user);
  db.query(
    queryString,
    [user.nume, user.prenume, user.email, user.datanastere, user.telefon, user.inaltime],
    (err, result) => {
      if (err) {
        callback(err);
      }
...
export const update = (user: User, callback: Function) => {
  ...
  db.query(queryString, [user.nume, user.prenume, user.telefon, user.email, user.inaltime, user.id], (err, result) => {
    if (err) {
      callback(err);
    }
    callback(null);
